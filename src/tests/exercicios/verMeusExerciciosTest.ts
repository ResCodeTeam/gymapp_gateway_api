...
describe("Teste Obter Exercicios do treinador logado", () => {
  ...
  describe("- Sem token", () => {
    it("Deve retornar erro de token invalido", () => {
      return chai
        .request(server)
        .get("/treinador/exercicios/treinador")
        .then((res) => {
        ...
        .then((res) => {
          ...
          if (res.body.length > 0) {
            ...
            if (res.body[0]["musculos"].length > 0) {
              ...
              chai
                .expect(res.body[0]["musculos"][0])
                .to.have.property("musculos");

              chai
                .expect(res.body[0]["musculos"][0]["musculos"])
                .to.be.an("array");

              if (res.body[0]["musculos"][0]["musculos"].length > 0) {
                chai
                  .expect(res.body[0]["musculos"][0]["musculos"][0])
                  .to.be.an("object");
                ...
                chai
                  .expect(res.body[0]["musculos"][0]["musculos"][0])
                  .to.have.property("img_url");

                chai
                  .expect(res.body[0]["musculos"][0]["musculos"][0]["nome"])
                  .to.be.a("string");
                chai
                  .expect(res.body[0]["musculos"][0]["musculos"][0]["img_url"])
                  .to.be.a("string");
              }
            }
          }
